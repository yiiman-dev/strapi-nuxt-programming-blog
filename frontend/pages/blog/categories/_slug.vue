          ...
          <div class="col-lg-5 col-md-12 text-lg-right md-mt-3">
            <nav aria-label="breadcrumb" class="page-breadcrumb">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                  <p-link to="/blog">{{ $t('blog') }}</p-link>
                </li>
                <li class="breadcrumb-item">{{ $t('categories') }}
                </li>
                <li class="breadcrumb-item">
                  <p-link :to="'/blog/categories/'+this.$route.params.slug">{{ category[0].name }}</p-link>
                </li>
              ...
              <div v-for="(item,index) in articles" :class="index===0?'post':'post mt-8'">
                ...
                <div class="post-desc">
                  ...
                  <div class="post-title">
                    <h4>
                      <p-link :to="'/blog/'+item.slug">{{ item.title }}</p-link>
                    </h4>
                  </div>
              ...
              <div class="widget">
                ...
                <ul class="widget-categories list-unstyled">
                  <li v-for="cat in categories">
                    <p-link :to="'/blog/categories/'+cat.slug">{{ cat.name }}</p-link>
                  </li>
                </ul>
              </div>
            </div>
...
export default {
  ...
  async asyncData({$strapi, params, i18n}) {
    return {
      articles: await $strapi.find('articles', {'categories.slug': params.slug, _locale: i18n.locale}),
      global: await $strapi.find('blogcategorypage', {_locale: i18n.locale}),
      categories: await $strapi.find('categories', {_locale: i18n.locale}),
      category: await $strapi.find('categories', {_locale: i18n.locale, slug: params.slug}),
    }
  },
  mounted() {
    main(this)
